...
import { formatInteger, formatLongDate, formatShortDate } from "@/lib/format";
...
interface StoreDailyTrafficChartProps {
  series: StoreDailySeries[];
  isLoading: boolean;
  monthLabel: string;
}

// Comportamiento diario del mes ancla, una línea por tienda — para comparar
// el ritmo día a día entre tiendas. Siempre TODAS las tiendas activas, sin
// importar el filtro de tienda de la página (ver plan-history).
export function StoreDailyTrafficChart({ series, isLoading, monthLabel }: StoreDailyTrafficChartProps) {
  const hasData = series.some((s) => s.points.some((p) => p.orderCount > 0));
  const rows = mergeByDate(series);

  return (
    <div className="rounded-xl border border-border bg-card p-4">
      <h2 className="text-sm font-medium text-foreground">Comportamiento por tienda</h2>
      ...
      {isLoading ? (
        ...
        <div className="flex h-72 items-center justify-center text-sm text-muted-foreground">
          Sin tráfico en el mes seleccionado.
        </div>
      ) : (
